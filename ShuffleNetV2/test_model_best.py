...
#model.load_state_dict(torch.load('./checkpoint-029.pth'), False) # 由于是不同环境训练的结果，所以需要加False
...
label_path = './valid-labels-107.txt'
test_path = './test.txt'
...
for i in range(length):
    ...
    path = item[0]
    label = item[1]
    label = label.reshape((1, 107)) # 把标签变成batch_size的形式，即使batch_size为1
