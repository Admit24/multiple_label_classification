...
#from annotation import MultiLabelParser as AnnotationParser # 用于制作标签信息

model_names = sorted(name for name in models.__dict__
                     if name.islower() and not name.startswith("__") and callable(models.__dict__[name]))

parser = argparse.ArgumentParser(description='PyTorch Multiple Label Training')
...
#parser.add_argument('--num_classes', default=107, type=int, help='num for label')
...
#        model = models.__dict__[args.arch]()
    ...
    args.batch_size = int(args.batch_size / ngpus_per_node) #，每张卡上的batchsize为总batchsize除以卡数
    model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[gpu]) # 将模型放到第i张卡上

    # define loss function (criterion) and optimizer
    criterion = nn.BCEWithLogitsLoss().cuda(gpu) # 多标签分类模型采用BCEloss
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        acc = validate(val_loader, model, criterion, gpu, args)
...
def train(train_loader, model, criterion, optimizer, epoch, gpu, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        acc, recall = accuracy(output, target)
        losses.update(loss.item(), images.size(0))
        top1.update(acc, images.size(0))
        top2.update(recall, images.size(0))
...
def validate(val_loader, model, criterion, gpu, args):
    ...
    with torch.no_grad():
        ...
        #print(' * Acc {top.avg:.3f} '.format(top=top1))
        print(' * Acc {top1.avg:.3f} Recall {top2.avg:.3f}'.format(top1=top1, top2=top2))
    return top1.avg


def save_checkpoint(state, is_best, filename='checkpoint_0_5.pth.tar'):
    torch.save(state, filename)
    if is_best:
        shutil.copyfile(filename, 'model_best_0_5.pth.tar')
...
def adjust_learning_rate(optimizer, epoch, args):
    ...
    lr = args.lr * (1 - epoch / args.epochs)  # 学习率从0.1开始线性衰减
